...
void imageFrame::sobel(imageFrame binImage)
{
	...
	for(int x= 1; x<imageH-1; x++)
	{
		for(int y =1; y< imageW-1; y++)
		{
			pixel_x = (sobel_x[0][0] * binImage.getPixelValue(x-1,y-1,0)) + (sobel_x[0][1] * binImage.getPixelValue(x-1,y,0)) + (sobel_x[0][2] * binImage.getPixelValue(x-1,y+1,0)) +
              (sobel_x[1][0] * binImage.getPixelValue(x,y-1,0))   + (sobel_x[1][1] * binImage.getPixelValue(x,y,0))   + (sobel_x[1][2] * binImage.getPixelValue(x,y+1,0)) +
              (sobel_x[2][0] * binImage.getPixelValue(x+1,y-1,0)) + (sobel_x[2][1] * binImage.getPixelValue(x+1,y,0)) + (sobel_x[2][2] * binImage.getPixelValue(x+1,y+1,0));
				 

            pixel_y = (sobel_y[0][0] * binImage.getPixelValue(x-1,y-1,0)) + (sobel_y[0][1] * binImage.getPixelValue(x-1,y,0)) + (sobel_y[0][2] * binImage.getPixelValue(x-1,y+1,0)) +
              (sobel_y[1][0] * binImage.getPixelValue(x,y-1,0))   + (sobel_y[1][1] * binImage.getPixelValue(x,y,0))   + (sobel_y[1][2] * binImage.getPixelValue(x,y+1,0)) +
              (sobel_y[2][0] * binImage.getPixelValue(x+1,y-1,0)) + (sobel_y[2][1] * binImage.getPixelValue(x+1,y,0)) + (sobel_y[2][2] * binImage.getPixelValue(x+1,y+1,0));
				
			value = sqrt((pixel_x* pixel_x)+ (pixel_y* pixel_y));; 
			// cout<< value<<endl;
			setPixelValue(value, x, y,0);
		}
	}
	
}

//Function to implement loG Operator 
void imageFrame::log(imageFrame binImage)
{
	// float loG[3][3] = {{0,-1,0},{-1,4,-1},{0,-1,0}};
	float loG[5][5] = {{0,0,-1,0,0},{0,-1,-2,-1,0},{-1,-2,16,-2,-1}, {0,-1,-2,-1,0}, {0,0,-1,0,0}};
          
    double pixel;    
	int value;
		
	for(int x= 2; x<imageH-2; x++)
	{
		
		for(int y =2; y< imageW-2; y++)
		{
			// pixel = (loG[0][0] * binImage.getPixelValue(x-1,y-1,0)) + (loG[0][1] * binImage.getPixelValue(x-1,y,0)) + (loG[0][2] * binImage.getPixelValue(x-1,y+1,0)) +
              // (loG[1][0] * binImage.getPixelValue(x,y-1,0))   + (loG[1][1] * binImage.getPixelValue(x,y,0))   + (loG[1][2] * binImage.getPixelValue(x,y+1,0)) +
              // (loG[2][0] * binImage.getPixelValue(x+1,y-1,0)) + (loG[2][1] * binImage.getPixelValue(x+1,y,0)) + (loG[2][2] * binImage.getPixelValue(x+1,y+1,0));
			

			pixel = ((loG[0][0] * binImage.getPixelValue(x-2,y-2,0)) + (loG[0][1] * binImage.getPixelValue(x-2,y-1,0)) + (loG[0][2] * binImage.getPixelValue(x-2,y,0))+
						 (loG[0][3] * binImage.getPixelValue(x-2,y+1,0)) + (loG[0][4] * binImage.getPixelValue(x-2,y+2,0)) + (loG[1][0] * binImage.getPixelValue(x-1,y-2,0))+
						 (loG[1][1] * binImage.getPixelValue(x-1,y-1,0)) + (loG[1][2] * binImage.getPixelValue(x-1,y,0)) + (loG[1][3] * binImage.getPixelValue(x-1,y+1,0))+
						 (loG[1][4] * binImage.getPixelValue(x-1,y+2,0)) + (loG[2][0] * binImage.getPixelValue(x,y-2,0)) + (loG[2][1] * binImage.getPixelValue(x,y-1,0))+
						 (loG[2][2] * binImage.getPixelValue(x,y,0)) + (loG[2][3] * binImage.getPixelValue(x,y+1,0)) + (loG[2][4] * binImage.getPixelValue(x,y+2,0))+
						 (loG[3][0] * binImage.getPixelValue(x+1,y-2,0)) + (loG[3][1] * binImage.getPixelValue(x+1,y-1,0)) + (loG[3][2] * binImage.getPixelValue(x+1,y,0))+
						 (loG[3][3] * binImage.getPixelValue(x+1,y+1,0)) + (loG[3][4] * binImage.getPixelValue(x+1,y+2,0)) + (loG[4][0] * binImage.getPixelValue(x+2,y-2,0))+
						 (loG[4][1] * binImage.getPixelValue(x+2,y-1,0)) + (loG[4][2] * binImage.getPixelValue(x+2,y,0)) + (loG[4][3] * binImage.getPixelValue(x+2,y+1,0))+
						 (loG[4][4] * binImage.getPixelValue(x+2,y+2,0)));
						
			// value = sqrt((pixel/4)*(pixel/4));
			 // value = sqrt((pixel)*(pixel));

			if(pixel <0)
			{
				setPixelValue(0,x, y,0);	
			}
			else
			{
				setPixelValue(1,x, y,0);
			}
			 // setPixelValue(value,x, y,0);
		}
	}
...
bool imageFrame::loGMorph(int i, int j)
{
	int value = getPixelValue(i,j,0);
	// cout<<value<<endl;
	int kernel = 1;
	if(value == 0){
	for(int row= i-kernel; row<= kernel +i; row++){
		for(int column = i-kernel; column<= kernel +i; column++){
		
				if(getPixelValue(row,column,0) == 1){return true;}
	...
	for(int row= i-kernel; row<= kernel +i; row++){
		for(int column = i-kernel; column<= kernel +i; column++){
				...
				if(getPixelValue(row,column,0) == 0){return true;}					
...
// void imageFrame::convolve(int i, int j, vector<double>  temp ){
	// int kernel = 1;
	// for(int row = i - kernel; row<= i + kernel ; row++){
		// for(int column  = j- kernel ; column<= j+kernel ; column++)
		// {
...
double imageFrame::featureEnergyExt(vector<double> temp){
	
	int kernel = 2;
	double value, energy;
	energy = 0;
	
	for(int row = 2 ; row< imageH -3; row++){
		for(int column = 2; column< imageW-3; column++){
			int k = 0;
			value = 0;
			for(int i = row - kernel ; i<= row+kernel; i++){
				for(int j = column- kernel; j<= column + kernel; j++){
					value += getPixelValue(i,j,0) *temp[k];
					k++;
	...
	energy /= (imageW*imageH);
	return energy;
}


//Function to texture segmentation 
vector<double> imageFrame::convolve(vector<double> filter){
	
	int kernel = 1,index;
	double value, energy;
	energy = 0;
	vector<double> convolutedImage;
	convolutedImage.resize(imageW* imageH* 1,0);
	for(int row = 1 ; row< imageH -1; row++){
		for(int column = 1; column< imageW-1; column++){
			int k = 0; value =0;
			for(int i = row - kernel ; i<= row+kernel; i++){
				for(int j = column- kernel; j<= column + kernel; j++){
					...
					value += getPixelValue(i,j,0) *filter[k];
					k++;
					
					
				}
			}
			index = row *imageW*1 + column* 1; 
			convolutedImage[index] = value;	
		}
	}
...
double imageFrame::euclid(vector<double> featureVec, vector<double> kmean){
	double dist = 0, tempDist;
	for(int i =0; i<9; i++){
		tempDist = pow((featureVec[i] - kmean[i]),2);
		dist += tempDist;
	}
...
void imageFrame::dcComponent(){
	double value;
	for(int row=0; row< imageH; row++){
		for(int column =0; column< imageW; column++){
			value += getPixelValue(row,column,0);
	...
	for(int row=0; row< imageH; row++){
		for(int column =0; column< imageW; column++){
			pixelValue = getPixelValue(row,column,0);
			setPixelValue(abs(value - pixelValue),row,column,0);
